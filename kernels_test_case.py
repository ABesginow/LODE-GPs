...
class TestKernel(unittest.TestCase):
    ...
    def test_deriv_term_extraction(self):
        ...
        result = prepare_asym_deriv_dict(left_poly, right_poly, context, var_dict = var_dict)
        import types
        # Result should be [[[<lambda>, 42], 3, 0]]
        self.assertIsInstance(result[0][0][0], types.FunctionType, "Creation of the lambda definitely failed")
        self.assertEqual(result[0][0][1], 42, "Coefficient wasn't initialized properly")
        self.assertEqual(result[0][1], 3, "Exponent 1 not correctly calculated")
        self.assertEqual(result[0][2], 0, "Exponent 2 not correctly calculated")
        # var1*num1*x^n
        ...
        left_poly = e**3*dx1
        right_poly = 1
        self.assertRaises(Exception, prepare_asym_deriv_dict, left_poly, right_poly, context, var_dict)
